...
The origin fit is frozen before scoring the next admitted surface: the target option quotes do not select that parameter vector. Conditioning on target spot and rates makes this a test of the transferred option law, not a joint ex ante forecast of the underlying asset, interest rates and option prices. Persistence is defined only where the origin surface can be interpolated; a missing benchmark value is not a model error. Comparisons must therefore retain their own admissible support rather than treating unmatched observations as zero losses.
