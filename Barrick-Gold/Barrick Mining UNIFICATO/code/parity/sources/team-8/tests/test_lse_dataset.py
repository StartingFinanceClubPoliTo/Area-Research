...
def sample_yields():
    maturities = {
        ...
        "US7Y": ("7Y", 2557, 4.40), "US20Y": ("20Y", 7305, 4.75),
        "US30Y": ("30Y", 10958, 4.80),
    }
    rows = []
...
def test_lse_curve_uses_latest_common_date_and_interpolates():
    ...
    assert set(US_TREASURY_TENORS).issubset(set(curve["symbol"]))
...
def test_lse_curve_uses_optional_long_tenors_in_current_nss_fit():
    curve = normalise_lse_yield_curve(sample_yields())
    assert {"US7Y", "US20Y", "US30Y"}.issubset(set(curve["symbol"]))
    assert "US10Y" not in set(curve["symbol"])
    assert curve["beta0"].nunique() == 1
    assert curve["tau1"].iloc[0] > 0.0
    assert curve["tau2"].iloc[0] > curve["tau1"].iloc[0]
    assert np.isfinite(curve["nss_residual_bp"]).all()
