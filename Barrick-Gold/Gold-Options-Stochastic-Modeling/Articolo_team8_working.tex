...
The LSE chain does not supply a risk-free curve. The reproducible build therefore states a flat continuously compounded annual rate of \(r=3.7\%\) as an input assumption and applies it to every expiry. This is not presented as an estimated term structure: it is a transparent normalization choice whose sensitivity should be tested in later snapshots.
...
\midrule
Exact constant-volatility Hawkes & LSE GLD option surface, COS pricing, no CIR variance block. & \(\sigma=0.2113\), \(\lambda_0=\bar{\lambda}=0.5121\), \(n=0.0104\), \(\mu_J=0.0653\), \(\sigma_J=0.1608\). Vega-weighted objective \(=8.352\cdot10^{-5}\). & Validates the exact Hawkes option-pricing transform, but it is not the final model because it removes stochastic variance. \\
...
\midrule
Black--Scholes & \(\sigma=0.2441\). & Positive volatility. \\
Heston & \(v_0=0.06811\), \(\kappa=1.74335\), \(\theta_v=0.04563\), \(\xi=0.28283\), \(\rho=0.18525\). & Feller gap \(=0.07910\). \\
Bates, Poisson jumps & \(v_0=0.06041\), \(\kappa=5.24508\), \(\theta_v=0.03858\), \(\xi=0.21086\), \(\rho=-0.02598\), \(\lambda=0.32520\), \(\mu_J=0.19602\), \(\sigma_J=0.03951\). & Feller gap \(=0.36025\). \\
Selected Bates--Hawkes & \(v_0=0.05532\), \(\kappa=5.23934\), \(\theta_v=0.04135\), \(\xi=0.40484\), \(\rho=-0.20426\), \(\lambda_0=0.34170\), \(\bar{\lambda}=0.24021\), \(n=0.05201\), \(\beta=9.29058\), \(\mu_J=0.15979\), \(\sigma_J=0.15483\). & Feller gap \(=0.26944\), \(n<1\). \\
\bottomrule
\end{tabularx}
...
\midrule
Black--Scholes & 0.713 & 0.924 & 96.45 & 128.67 & Baseline fit is too rigid for the observed smile. \\
Heston & 0.443 & 0.581 & 63.60 & 82.19 & Stochastic variance improves on the constant-volatility baseline. \\
Bates, Poisson jumps & 0.446 & 0.611 & 59.59 & 81.73 & Adds tail-risk structure through independent arrivals. \\
Selected Bates--Hawkes & 0.300 & 0.459 & 41.74 & 60.10 & Best in-sample fit in the current LSE constrained comparison; self-excitation remains stationary. \\
\bottomrule
\end{tabularx}
...
These numbers explain the modelling choice. The current LSE full Bates--Hawkes run is the in-sample scalar-error winner while preserving a positive Feller gap and a stationary branching ratio. Its stationary mean intensity, (0.2534), has the same order of magnitude as the Bates intensity, (0.3252), while the timing of arrivals becomes state dependent. Within the Hawkes family, the exponential-kernel route is preferred over the exact constant-volatility and rough-kernel routes because it is simultaneously option-calibrated, Feller-admissible, Hawkes-stable, and usable as a self-exciting path generator. The result remains a single-cross-section fit and therefore requires the residual and sensitivity checks in Chapter~7.
...
\midrule
GBM / BS & 483.582 & 279.241 & 166.548 & 421.868 & 1005.789 & -0.027 & -0.052 & 0.000 \\
Heston & 484.554 & 267.479 & 194.628 & 420.326 & 963.235 & 0.130 & 0.246 & 0.000 \\
Bates & 481.492 & 275.450 & 190.179 & 418.562 & 982.204 & 0.131 & 0.174 & 1.625 \\
Full Bates--Hawkes & 486.063 & 290.891 & 177.662 & 418.758 & 1023.044 & 0.070 & 0.272 & 1.284 \\
\bottomrule
\end{tabular}
...
From a modelling perspective, the pipeline shows why diagnostics are indispensable. A scalar objective value is not enough. The fitted model must be inspected through residual heatmaps, smile slices, parameter plausibility, model-comparison tables, and path-level distributional checks. In the current LSE deterministic run, the full Bates--Hawkes model reduces price RMSE from (0.6114) for Bates to (0.4589) and implied-volatility RMSE from (81.73) to (60.10) basis points. Its Feller gap is (0.26944), and the branching ratio (n=0.0520) is stationary and above the imposed minimum. These results support the self-exciting layer as an in-sample description of this GLD cross-section. They do not establish out-of-sample dominance or a time-series clustering premium without additional dates. In the five-year simulation, the full model combines Heston stochastic variance with event-driven Hawkes jumps and keeps the state variables visible for downstream validation.

The final purpose of the article is therefore to propose and test gold-price models for the Barrick research pipeline. Previous work supplies the Monte Carlo, quasi-Monte Carlo, and variance-reduction framework, and earlier company-level modelling explains why future valuation work will need gold-price scenarios \cite{BarrickArticle4,BarrickArticle5}. The present article uses those numerical tools as inputs for an applied path generator and makes the model-specific random blocks explicit. The current LSE chain is the sole empirical input; its raw rows remain local, while the reproducible transformation and aggregate outputs are published. The revenue, EBITDA, and equity-valuation model is not built here: it belongs to a later unified article. The principal extension is the full affine Bates--Hawkes model; proxy, constant-volatility, event-likelihood, and rough-kernel approaches are retained as clearly separated alternatives rather than presented as the article's objective.
