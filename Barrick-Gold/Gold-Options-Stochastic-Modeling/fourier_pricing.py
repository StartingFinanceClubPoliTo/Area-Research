"""Reusable Fourier inversion kernels for Bates and Bates--Hawkes models."""
...
def carr_madan_call(charfunc, spot, strike, maturity, rate, dividend_yield=0.0):
    """Price one European call with the repository's Lewis/Carr--Madan formula."""
    if maturity <= 0.0:
...
def gil_pelaez_call_put(charfunc, spot, strike, maturity, rate, dividend_yield=0.0):
    """Return call and put prices through independent Gil--Pelaez inversions."""
    if maturity <= 0.0:
...
def cos_call_prices_from_values(phi_values, frequencies, lower, upper, strikes, rate, maturity):
    """Evaluate COS call payoffs for many strikes from one CF grid evaluation."""
...
def adaptive_cos_call_prices(
    ...
    """Price a strike vector after estimating a stable two-cumulant range."""
    strike_array = np.atleast_1d(np.asarray(strikes, dtype=float))
    if maturity <= 0.0:
    ...
    second_cumulant = max(
        -2.0 * log_phi.real / cumulant_step ** 2,
        1e-6,
    )
